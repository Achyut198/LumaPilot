...
class AboutPrefsViewController: NSViewController, SettingsPane {
  ...
  @IBAction func openDonate(_: NSButton) {
    if let url = URL(string: "https://lumapilot.app/donate") {
      NSWorkspace.shared.open(url)
    }
  }

  @IBAction func openWebPage(_: NSButton) {
    if let url = URL(string: "https://lumapilot.app") {
      NSWorkspace.shared.open(url)
    }
  }

  @IBAction func openSupportPage(_: NSButton) {
    if let url = URL(string: "https://lumapilot.app/support") {
      NSWorkspace.shared.open(url)
    }
  }
  ...
  func setCopyrightInfo() {
    let copyright = NSLocalizedString("Copyright Ⓒ LumaPilot, ", comment: "Version")
