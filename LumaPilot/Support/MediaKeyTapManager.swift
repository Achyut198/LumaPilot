...
class MediaKeyTapManager: MediaKeyTapDelegate {
  ...
  static func acquirePrivileges(firstAsk: Bool = false) {
    if !self.readPrivileges(prompt: true), !firstAsk {
      ...
      alert.informativeText = NSLocalizedString("You need to enable LumaPilot in System Settings > Security and Privacy > Accessibility for the keyboard shortcuts to work", comment: "Shown in the alert dialog")
      alert.runModal()
    }
  }
